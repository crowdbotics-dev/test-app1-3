...
const inviteUserToChannelsSlice = createSlice({
  ...
  extraReducers: builder => {
    builder.addCase(modules_slack_service_invite_user_to_channel_create.pending, (state, action) => {
      if (state.api.loading === "idle") {
        state.api.loading = "pending";
      }
    }).addCase(modules_slack_service_invite_user_to_channel_create.fulfilled, (state, action) => {
      if (state.api.loading === "pending") {
        state.entities.push(action.payload);
        state.api.loading = "idle";
      }
    }).addCase(modules_slack_service_invite_user_to_channel_create.rejected, (state, action) => {
      if (state.api.loading === "pending") {
