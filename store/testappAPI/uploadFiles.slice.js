...
const uploadFilesSlice = createSlice({
  ...
  extraReducers: builder => {
    builder.addCase(modules_drive_service_upload_file_create.pending, (state, action) => {
      if (state.api.loading === "idle") {
        state.api.loading = "pending";
      }
    }).addCase(modules_drive_service_upload_file_create.fulfilled, (state, action) => {
      if (state.api.loading === "pending") {
        state.entities.push(action.payload);
        state.api.loading = "idle";
      }
    }).addCase(modules_drive_service_upload_file_create.rejected, (state, action) => {
      if (state.api.loading === "pending") {
